...
class DataLoader:
    ...
    def sample(self):
        ...
        output_label_arr = np.zeros((self.mbsz, output_lens[0])).astype('int32')
        pos = 0
        for i in xrange(self.mbsz):
            label_arr[pos:pos+output_lens[i]] = outputs[i]
            pos += output_lens[i]
            output_label_arr[i,:] = outputs[i]
        label_mask = np.ones(((self.mbsz, len(outputs[i])))).astype('float32')

        return np.array(input_arr).astype('float32'), output_label_arr, label_mask
