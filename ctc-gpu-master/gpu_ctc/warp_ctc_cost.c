...
int APPLY_SPECIFIC(gpu_ctc_cost)(CudaNdarray* probs,  
							...
							CudaNdarray** costs)
{
	...
	CudaNdarray* grads = new CudaNdarray;
	grads->devdata = NULL;
	CudaNdarray* my_costs = (CudaNdarray*)CudaNdarray_ZEROS(1, cost_dims);

	if (CudaNdarray_prep_output(costs, 1, CudaNdarray_HOST_DIMS(my_costs)) != 0)
	{
		PyErr_SetString(PyExc_RuntimeError, "error prep output for costs");
        return 1;
	}
    ...
    if (CudaNdarray_CopyFromCudaNdarray(*costs, my_costs))
	{
		PyErr_SetString(PyExc_RuntimeError, "cudacopying");
        return 1;
	}
